...
static void source_send(struct source *self, int fd) {
    // unset O_NONBLOCK
    fcntl(fd, F_SETFL, 0);
    if (self->data_to_copy != NULL) {
        ...
        const char * const *dataptr = self->data_to_copy;
        ...
        if (fork() == 0) {
            ...
            execlp("cat", "cat", self->temp_file_to_copy, NULL);
            perror("exec cat");
            exit(1);
        }
        close(fd);
...
void source_offer(struct source *self, char *mime_type) {
    if (mime_type == NULL || mime_type_is_text(mime_type)) {
        ...
        self->do_offer(self->proxy, text_plain);
        self->do_offer(self->proxy, text_plain_utf8);
        self->do_offer(self->proxy, "TEXT");
        self->do_offer(self->proxy, "STRING");
        self->do_offer(self->proxy, "UTF8_STRING");
    }
    if (mime_type != NULL) {
...
#define INIT(type) \
void init_ ## type(struct source *self) { \
    self->do_offer = \
